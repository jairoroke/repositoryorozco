...
#include "AP_RangeFinder_I2C_Sensors.h"


__s32 I2CSensors::i2c_init (void)
{
...
__s32 I2CSensors::sonnar_connect(__u8 sonarAddress)
{
  //

  if (ioctl(file_i2c, I2C_SLAVE, sonarAddress) < 0)
  {
...
void I2CSensors::sonarTakeRange(__u8 sonarAddress)
{
  __u8 buffer[2] = {0};
  buffer[0] = sonarAddress;
  buffer[1] = MAXSONAR_UPDATEREAD;
  int length = 2;

  sonnar_connect(sonarAddress);
...
__u16 I2CSensors::sonarGetRangeRead(__u8 sonarAddress)
{

  __u8 buffer[2] = {0};
  buffer[0] = sonarAddress;
  buffer[1] = MAXSONAR_GETREAD;
  int length = 2;
  __u16 reading_cm = 0;
  ...
  {
    reading_cm = sonari2cRead(sonarAddress);
  }
...
__u16 I2CSensors::sonari2cRead(__u8 sonarAddress)
{
  int length = 2;
  __u8 buffer[2];
  __u16 reading_cm = 0;
...
------------------------------------------------------------------------------*/
__s32 I2CSensors::garmin_connect (__u8 lidarliteAddress)
{
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
void I2CSensors::garmin_configure(__u8 configuration, __u8 lidarliteAddress)
{
    __u8 sigCountMax;
    __u8 acqConfigReg;
    __u8 refCountMax;
    ...
    garmini2cWrite(LLv3_SIG_CNT_VAL,   &sigCountMax    , 1, lidarliteAddress);
    garmini2cWrite(LLv3_ACQ_CONFIG,    &acqConfigReg   , 1, lidarliteAddress);
    garmini2cWrite(LLv3_REF_CNT_VAL,   &refCountMax    , 1, lidarliteAddress);
    garmini2cWrite(LLv3_THRESH_BYPASS, &thresholdBypass, 1, lidarliteAddress);
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
void I2CSensors::garminTakeRange(__u8 lidarliteAddress)
{
    __u8 commandByte = 0x04;

    garmini2cWrite(LLv3_ACQ_CMD, &commandByte, 1, lidarliteAddress);
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
void I2CSensors::garminWaitForBusy(__u8 lidarliteAddress)
{
    __u8  busyFlag;

    do  // Loop until device is not busy
    {
        busyFlag = garminGetBusyFlag(lidarliteAddress);
    } while (busyFlag);
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
__u8 I2CSensors::garminGetBusyFlag(__u8 lidarliteAddress)
{
    __u8  statusByte = 0;
    __u8  busyFlag; // busyFlag monitors when the device is done with a measurement

    // Read status register to check busy flag
    garmini2cRead(LLv3_STATUS, &statusByte, 1, lidarliteAddress);
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
__u16 I2CSensors::garminReadDistance(__u8 lidarliteAddress)
{
    __u8  distBytes[2] = {0};

    // Read two bytes from register 0x0f and 0x10 (autoincrement)
    garmini2cRead((LLv3_DISTANCE | 0x80), distBytes, 2, lidarliteAddress);
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
__s32 I2CSensors::garmini2cWrite(__u8 regAddr,  __u8 * dataBytes,
                             __u8 numBytes, __u8 lidarliteAddress)
{
    ...
    garmin_connect(lidarliteAddress);
...
/*------------------------------------------------------------------------------
  ...
  operating manual for instructions.
------------------------------------------------------------------------------*/
__s32 I2CSensors::garmini2cRead(__u8 regAddr,  __u8 * dataBytes,
                            __u8 numBytes, __u8 lidarliteAddress)
{
    __u8 buffer;
...
/*------------------------------------------------------------------------------
  ...
  lidarliteAddress: Default 0x62. Fill in new address here if changed. See
    operating manual for instructions.
------------------------------------------------------------------------------*/
void I2CSensors::correlationRecordRead(__s16 * correlationArray,
                                         ...
                                         __u8  lidarliteAddress)
{
    ...
    //  Select memory bank
    dataBytes[0] = 0xc0;
    garmini2cWrite(LLv3_ACQ_SETTINGS, dataBytes, 1, lidarliteAddress);

    // Test mode enable
    dataBytes[0] = 0x07;
    garmini2cWrite(LLv3_COMMAND, dataBytes, 1, lidarliteAddress);

    for (i=0 ; i<numberOfReadings ; i++)
    {
        garmini2cRead((LLv3_CORR_DATA | 0x80), dataBytes, 2, lidarliteAddress);
